...
| 013 | `RentalRepository` — fundação | Só o Repository (dono de `List<Rental>`, seed idêntico ao atual) + `AppState` proxy (mesmo padrão de `ToyRepository`/`BusinessSettingsRepository`) — **sem migrar nenhuma tela/Cubit ainda**. | Baixo — puramente estrutural, nenhum consumidor novo ainda |
| 014 | Relatório | `report_tab.dart` → `Cubit` fino de leitura sobre `RentalRepository` (filtro de período, totais, breakdown) | Baixo — só leitura, sem escrita/timer/notificação |
| 015 | Catálogo — grade do catálogo | `catalog_tab.dart` → `ToyCatalogCubit` (já existe, da 012) passa a também expor disponibilidade via um pequeno `ComputeToyAvailability` (use case cross-repository, `ToyRepository` + `RentalRepository`) | Médio |
| 016 | Serviços de locação — fundação | `lib/services/pix_payload.dart` + `lib/notifications/*` → `lib/data/services/` (só mover — já são funções puras/interface injetável, sem `Cubit`/`View` ainda) | Baixo — mecânico, mesmo espírito da fundação 013 |
| 017 | Nova locação | `new_rental_sheet.dart` → `NewRentalCubit` + View, rascunho próprio (não compartilhado com `AppState.draft`) | Médio — primeira fatia com estado de UI genuinamente paralelo ao de `AppState` |
| 018 | Locação ativa, estender, cancelar, encerrar + Pix | `active_tab.dart` + `end_rental_dialog.dart` + `pix_qr_sheet.dart` (migrados juntos — compartilham a máquina de estado do diálogo de encerramento) → `ActiveRentalsCubit` + Views | Alto — timers, notificação local, Pix |
| 019 | Painel do dia (Home) | `home_tab.dart` → `HomeCubit` + View, cross-repository como o `ReportCubit` (014) | Baixo — só leitura, sem consumidor fora de escopo |

Cada fatia só começa quando a anterior estiver `Implemented` (constitution: zero-breakage, sem duas fatias arquiteturais em paralelo). Números exatos confirmados na hora de criar cada pasta, respeitando a numeração sequencial do `specs/README.md`.

**Roadmap de locação concluído em 019** (2026-08-24): nenhuma `View` de tela lê `AppState` diretamente mais. Sobrou só: (a) `AppState.tab`/`setTab`/`screenTitle`/`kicker` — estado de navegação de shell (`home_shell.dart`, `app_bottom_nav.dart`, `app_header.dart`), nunca fez parte de nenhuma spec deste roadmap (é sobre qual aba está selecionada, não sobre nenhum domínio migrado); (b) os métodos que ~30 arquivos de teste chamam direto em `AppState` como atalho de setup (`submitNew`, `extendActive`, `confirmEnd`, etc.) — mantidos pela regra de não-quebra da constitution, não uma pendência de migração.

**Aprendizado da fatia 012** (2026-08-23, ajuste permitido pela dúvida não-bloqueante já registrada abaixo): `catalog_tab.dart` (a grade com os "tickets" de disponibilidade) chama `AppState.toyAvailable(Toy)`, que cruza `Toy.qty` com `rentals` ativas — uma consulta genuinamente cross-repository (`Toy` + `Rental`), não resolvível só com `ToyRepository`. Decisão: `add_toy_sheet.dart` (criação pura de brinquedo, sem tocar em locação) migra na 012; `catalog_tab.dart` só migra depois que `RentalRepository` existir.

**Aprendizado da fatia 013** (2026-08-23): mesmo problema se repetiu com `report_tab.dart` — é **inteiramente** derivado de `Rental` (`paymentBreakdown`, `reportTotal`, `toyBreakdown`, `historyList`, `reportFiltered`), não dá pra virar fatia própria antes de `RentalRepository` existir. Por isso a antiga "013 Relatório" virou "014", e uma fatia nova e menor ("013") nasce só pra criar `RentalRepository` como fundação — mesmo padrão da 010 (fundação sem tela), aplicado agora ao domínio `Rental`. As fatias que seguem (014 Relatório, 015 grade do catálogo, 016 Locação) constroem em cima dela.

**Aprendizado da fatia 016** (2026-08-23): a antiga "016 Locação (nova/ativa/encerrar) + PIX + notificações" era grande demais pra uma fatia só (a própria dúvida original já previa isso). Decisão: sub-fatiar. Primeiro passo é uma fundação mecânica — `lib/services/pix_payload.dart` e `lib/notifications/*` já são funções puras / uma interface (`RentalNotifier`) já injetável (usada por fake em teste desde a spec 005) — só precisam mudar de pasta pra `lib/data/services/`, sem `Cubit`/`View` ainda, mesmo espírito da fundação 013. As sub-fatias seguintes (nova locação, ativa/estender/cancelar, encerrar+Pix+notificação) constroem em cima dela — números exatos definidos quando cada uma for criada, não fixados de antemão.
