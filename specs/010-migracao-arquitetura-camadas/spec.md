...
| 017+ | Locação (nova/ativa/encerrar) + PIX + notificações | `home_tab.dart`, `active_tab.dart`, `new_rental_sheet.dart`, `end_rental_dialog.dart`, `pix_qr_sheet.dart` → `Cubit`(s) de locação sobre `RentalRepository` + os `Service`s da 016 | Alto — timers, notificação local, PIX. Sub-fatiado (ver "Aprendizado da fatia 016" abaixo); números/escopo exatos de cada sub-fatia definidos quando ela for criada. |
